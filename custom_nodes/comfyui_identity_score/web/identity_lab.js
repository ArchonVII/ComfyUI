...
function safeAction(status, operation) { return async (...args) => { try { return await operation(...args); } catch (error) { status.textContent = `Error: ${String(error.message || error)}`; } }; }
