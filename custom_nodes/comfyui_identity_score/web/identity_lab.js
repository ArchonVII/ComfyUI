...
const ROLE_TYPES = {
  ...
  IDENTITY_LAB_SAMPLER: ["KSampler", "KSamplerAdvanced"],
...
function parseSetup(raw) {
  ...
  const loras = (raw.loras ?? []).filter((item) => item?.name?.trim()).slice(0, 3).map((item) => ({
    name: item.name.trim(), strength: strictNumber(item.strength, "LoRA strength", { min: -4 }),
  }));
  return {
    mode: raw.mode === "identity_i2i" ? "identity_i2i" : "face_swap",
...
class SerialQueue {
  constructor({ fetchApi = api.fetchApi.bind(api), graphToPrompt = app.graphToPrompt.bind(app), onUpdate = () => {}, pollMs = 2000 } = {}) {
    Object.assign(this, { fetchApi, graphToPrompt, onUpdate, pollMs, paused: false, running: false });
  }
  ...
  async run(experimentId) {
    ...
    try {
      const capture = await this.graphToPrompt(); const workflow = capture.output ?? capture.prompt ?? capture;
      while (!this.paused) {
        ...
        await submitOne({ fetchApi: this.fetchApi, workflow, experimentId, run, settings: detail.experiment.settings.setup ?? {} });
        await this.waitForTerminal(experimentId, run.id);
      }
    } catch (error) { this.onUpdate({ error: String(error.message || error) }); }
    finally { this.running = false; }
  }
  async waitForTerminal(experimentId, runId) {
    for (;;) {
      const detail = await responseJson(await this.fetchApi(`${LAB_ROOT}/experiments/${experimentId}`));
...
function activeScore(run) { const value = run.identity_report?.active_score ?? run.identity_report?.active?.score; return Number.isFinite(value) ? value : -Infinity; }
function galleryMetadata(run) {
  const report = run.identity_report ?? {}; const plan = run.plan ?? {}; const detection = report.face_detection ?? {};
  const loras = (plan.loras ?? []).map((item) => Array.isArray(item) ? `${item[0]} @ ${item[1]}` : `${item.name} @ ${item.strength}`).join(", ") || "no LoRA";
  return `active ${activeScore(run)} • reference ${report.reference_score ?? "—"} • base ${report.base_score ?? "—"} • ${report.rankable ? "rankable" : "not rankable"} • detections base:${!!detection.base} reference:${!!detection.reference} generated:${!!detection.generated} • ${plan.checkpoint ?? "—"} • ${loras} • steps ${plan.steps ?? "—"} cfg ${plan.cfg ?? "—"} ${plan.sampler ?? "—"}/${plan.scheduler ?? "—"} denoise ${plan.denoise ?? "—"} • runtime ${report.runtime_seconds ?? "—"}`;
}
...
function renderGallery(container, detail, refresh, selected = new Set(), filters = {}) {
  ...
  [["stage", values("stage")], ["checkpoint", values("checkpoint")], ["state", [...new Set(allRuns.map((run) => run.state))]]].forEach(([key, options]) => toolbar.append(selectFilter(key, options, (event) => { filters[key] = event.target.value; rerender(); })));
...
function renderPanel(container) {
  ...
  form.addEventListener("submit", async (event) => { event.preventDefault(); try {
    ...
    const id = created.experiment.id; const pause = el("button", "Pause after current"); pause.addEventListener("click", () => queue?.pause()); const resume = el("button", "Resume confirmed stale work"); resume.addEventListener("click", () => queue?.resume(id)); const promote = el("button", "Promote selected to refine"); promote.addEventListener("click", async () => { const detail = await refresh(id); const candidates = detail.runs.filter((run) => selected.has(run.id)); if (!candidates.length) throw new Error("select result candidates before promotion"); await responseJson(await api.fetchApi(`${LAB_ROOT}/experiments/${id}/promote`, { method: "POST", body: JSON.stringify({ checkpoints: [...new Set(candidates.map((run) => run.plan.checkpoint))], seeds: [...new Set(candidates.map((run) => run.plan.seed))], loras: [], stages: ["refine"], refine_settings: { steps: settings.steps, cfg: settings.cfg, sampler: settings.sampler, scheduler: settings.scheduler, denoise: settings.denoise, pixel_budget: settings.pixelBudget } }) })); await refresh(id); }); const archive = el("button", "Archive"); archive.addEventListener("click", async () => { await responseJson(await api.fetchApi(`${LAB_ROOT}/experiments/${id}/archive`, { method: "POST", body: "{}" })); status.textContent = "Archived. Outputs retained."; }); const preview = el("button", "Preview deletion"); const confirmation = el("input"); confirmation.placeholder = "Paste exact delete confirmation"; const remove = el("button", "Delete archived experiment"); preview.addEventListener("click", async () => { const value = await responseJson(await api.fetchApi(`${LAB_ROOT}/experiments/${id}/delete-preview`)); confirmation.placeholder = value.confirmation; status.textContent = `Delete preview: ${value.runs.length} database rows; ${value.files.join(", ") || "no output files"}`; }); remove.addEventListener("click", async () => { const value = await responseJson(await api.fetchApi(`${LAB_ROOT}/experiments/${id}`, { method: "DELETE", body: JSON.stringify({ confirmation: confirmation.value }) })); status.textContent = `Deleted ${value.runs.length} rows and ${value.files.length} files.`; gallery.replaceChildren(); }); actions.replaceChildren(pause, resume, promote, archive, preview, confirmation, remove);
...
const seam = { parseSetup, estimatePreview, patchPrompt, submitOne, filterAndSortResults, galleryMetadata, SerialQueue, renderPanel };
globalThis.__identityLab = seam;
export { parseSetup, estimatePreview, patchPrompt, submitOne, filterAndSortResults, galleryMetadata, SerialQueue };
