...
function renderGallery(container, detail, refresh, selected = new Set(), filters = {}, status) {
  ...
  for (const run of results) {
    ...
    if (run.state === "completed") for (let rating = 1; rating <= 5; rating++) { const button = el("button", String(rating)); button.addEventListener("click", safeAction(status, async () => { await responseJson(await api.fetchApi(`${LAB_ROOT}/runs/${run.id}/review`, { method: "PATCH", body: JSON.stringify({ rating }) })); refresh(); })); controls.append(button); }
    const favorite = el("button", run.favorite ? "★" : "☆"); favorite.addEventListener("click", safeAction(status, async () => { await responseJson(await api.fetchApi(`${LAB_ROOT}/runs/${run.id}/review`, { method: "PATCH", body: JSON.stringify({ favorite: !run.favorite }) })); refresh(); })); controls.append(favorite); card.append(controls); container.append(card);
    if (run.state === "completed") { const reject = el("button", "Reject (1)"); reject.addEventListener("click", safeAction(status, async () => { await responseJson(await api.fetchApi(`${LAB_ROOT}/runs/${run.id}/review`, { method: "PATCH", body: JSON.stringify({ rating: 1, favorite: false }) })); refresh(); })); const notes = el("textarea"); notes.value = run.notes ?? ""; notes.placeholder = "Local review notes"; notes.addEventListener("change", safeAction(status, async () => { await responseJson(await api.fetchApi(`${LAB_ROOT}/runs/${run.id}/review`, { method: "PATCH", body: JSON.stringify({ notes: notes.value }) })); })); card.append(reject, notes); }
  }
}
...
function renderPanel(container) {
  ...
  form.addEventListener("submit", async (event) => { event.preventDefault(); try {
    ...
    queue = new SerialQueue({ onUpdate: (detail) => { if (detail.runs) { status.textContent = statusSummary(detail); renderGallery(gallery, detail, () => refresh(id), selected, {}, status); } } }); refresh(id); queue.run(id);
