...
const backend = {fetchApi: async (path, options = {}) => {
  ...
  if (path === "/identity-lab/experiments/exp-1") return reviewRefreshFails ? textFailure("detail refresh failure") : ok(detail());
  if (path.endsWith("/review")) return reviewRefreshFails ? ok(run) : jsonFailure("review JSON detail");
...
let sidebar, unhandled = [], reviewRefreshFails = false;
...
reviewRefreshFails = true;
await button("4").listeners.click();
await new Promise((resolve) => setTimeout(resolve, 0));
assertStatus("detail refresh failure");
reviewRefreshFails = false;
