...
const bodies=calls.map(([p,o])=>[p,o.method, o.body&&JSON.parse(o.body)]); if(!bodies.some(([p,m,b])=>p.includes("/review")&&b.rating===5)||!bodies.some(([p,m,b])=>p.includes("/review")&&b.favorite===true)||!bodies.some(([p,m,b])=>p.includes("/review")&&b.notes==="keep details"))throw new Error("review controls did not patch local API"); if(!bodies.some(([p,m,b])=>p.endsWith("/promote")&&b.stages[0]==="focused_refine"&&b.refine_settings.steps===31))throw new Error("focused refinement used stale controls"); if(!archived||!deletePreview||!deleted||!bodies.some(([p,m,b])=>m==="DELETE"&&b.token==="a".repeat(64)))throw new Error("archive/delete contract was not driven");
