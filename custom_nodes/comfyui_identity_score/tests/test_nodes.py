...
def test_dual_identity_score_records_identified_experiment_runs_without_failing_no_face_results(monkeypatch):
    report = {
        "reference_to_output": {"cosine_similarity": None, "same_identity": False},
        "base_to_output": {"cosine_similarity": 0.84, "same_identity": True},
        "active_score": {"source": "reference", "cosine_similarity": None, "same_identity": False},
    ...
    class RecordingService:
        def record_run(self, **kwargs):
            calls.append(kwargs)
            return {"id": "run-2", "output_path": "identity_lab/results/run-2.png"}

    monkeypatch.setattr(nodes, "build_dual_report", lambda **_kwargs: report)
    monkeypatch.setattr(nodes, "image_tensor_to_bgr", lambda image: image)
    monkeypatch.setattr(nodes, "get_experiment_service", lambda: RecordingService(), raising=False)

    result = nodes.DualIdentityScore().score_identity(
        base_image="base", reference_image="reference", generated_image="generated", experiment_mode="face_swap",
        face_score_threshold=0.7, same_identity_threshold=0.363, face_selection="largest", write_manifest=False,
        manifest_dir="default/identity_score_runs", run_label="dual-score", metadata_key="identity_score_report",
        experiment_id="experiment-1", run_id="run-2", prompt={"prompt": "x"}, extra_pnginfo={"workflow": {}},
    )

    assert calls and calls[0]["experiment_id"] == "experiment-1"
    assert calls[0]["run_id"] == "run-2"
    assert calls[0]["report"]["rankable"] is False
    saved_report = json.loads(result["result"][10])
    assert saved_report["result_path"] == "identity_lab/results/run-2.png"
    assert result["ui"]["result_path"] == ["identity_lab/results/run-2.png"]


def test_dual_identity_score_keeps_manual_scoring_isolated_from_experiment_database(monkeypatch):
    report = {
        "reference_to_output": {"cosine_similarity": 0.9, "same_identity": True},
        "base_to_output": {"cosine_similarity": 0.4, "same_identity": True},
        "active_score": {"source": "reference", "cosine_similarity": 0.9, "same_identity": True},
        "rankable": True,
        "face_detection": {"base": True, "reference": True, "generated": True},
        "issues": [],
    }
    monkeypatch.setattr(nodes, "build_dual_report", lambda **_kwargs: report)
    monkeypatch.setattr(nodes, "image_tensor_to_bgr", lambda image: image)
    monkeypatch.setattr(nodes, "get_experiment_service", lambda: pytest.fail("manual scoring must not create a service"), raising=False)

    nodes.DualIdentityScore().score_identity(
        base_image="base", reference_image="reference", generated_image="generated", experiment_mode="face_swap",
        face_score_threshold=0.7, same_identity_threshold=0.363, face_selection="largest", write_manifest=False,
        manifest_dir="default/identity_score_runs", run_label="dual-score", metadata_key="identity_score_report",
    )
